...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <motion.div 
      ...
      viewport={{ once: true }}
      className="group relative bg-[#FDFBF9] shadow-[0_20px_50px_rgba(0,0,0,0.02)] hover:shadow-premium-2xl rounded-[3.5rem] overflow-hidden transition-all duration-1000 border border-white/80 p-4 hover:-translate-y-4"
    >
      <Link to={`/product/${product.id}`} className="block relative aspect-[4/5] overflow-hidden rounded-[2.8rem] mb-8 bg-cream">
        <motion.img 
        ...
        <div className="absolute top-6 left-6 z-20">
           <div className="bg-white/90 backdrop-blur-md px-4 py-2 rounded-full border border-white/60 shadow-premium flex items-center gap-2">
        ...
        <div className="absolute top-6 right-6 z-20">
          <button 
            ...
            className={cn(
              "w-10 h-10 rounded-full transition-all duration-1000 flex items-center justify-center backdrop-blur-md shadow-premium border",
        ...
        <div className="absolute bottom-8 left-6 right-6 translate-y-12 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-700 z-10">
          <button 
            onClick={handleAddToCart}
            className={cn(
              "w-full py-4 rounded-full flex items-center justify-center gap-4 text-[10px] font-black uppercase tracking-[0.5em] transition-all duration-700 italic shadow-premium border border-white/20",
      ...
      <div className="px-5 pb-4 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5 px-3 py-1 bg-white shadow-premium rounded-full border border-coffee-50/50">
        ...
        <div className="space-y-3">
          <Link to={`/product/${product.id}`} className="block">
            <h3 className="text-xl md:text-2xl lg:text-3xl font-display font-black text-espresso italic group-hover:text-caramel transition-colors tracking-tightest leading-[1.1] uppercase">
              {product.name}
            </h3>
          </Link>
          
          <div className="flex items-center gap-4 overflow-hidden">
             <div className="h-[2px] w-8 bg-coffee-50 shrink-0" />
        ...
        <div className="pt-2 flex flex-col md:flex-row md:items-center justify-between gap-3">
          <div className="flex flex-col">
             <span className="text-2xl md:text-3xl font-display font-black text-espresso tracking-tightest italic leading-none">
               {formatPrice(lbp)}
             </span>
             {usd > 0 && (
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to={`/product/${product.id}`} className="w-10 h-10 bg-cream text-espresso hover:bg-espresso hover:text-white rounded-full flex items-center justify-center transition-all duration-700 shadow-premium group/arrow">
