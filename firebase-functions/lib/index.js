...
export const aiChat = onRequest({ cors: true, maxInstances: 10 }, async (req, res) => {
...
If they ask about subscription plans, suggest Daily Essentials (our daily delivery plan), Starter, or Premium plans.`;
        if (context) {
            ...
            if (context.userEmail)
                parts.push(`User's Email: ${context.userEmail}`);
            ...
            if (Array.isArray(context.products) && context.products.length > 0) {
                parts.push(`Available Products: ${context.products.map((p) => `${p.name} ($${p.priceUsd || p.price})`).join(', ')}`);
            }
            if (Array.isArray(context.plans) && context.plans.length > 0) {
                parts.push(`Subscription Plans: ${context.plans.map((p) => `${p.name} (Frequency: ${p.frequency})`).join(', ')}`);
            }
            if (parts.length > 0) {
